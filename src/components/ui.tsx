...
export function LoadingScreen({label='กำลังเตรียมระบบ',admin=false}:{label?:string;admin?:boolean}){
 return <main className={`loading-screen ${admin?'admin-loading':''}`} aria-busy="true" aria-live="polite"><div className="loading-glow one"/><div className="loading-glow two"/><section className="loading-card"><Logo/><span className="loading-eyebrow">{admin?'MAKE IT CONTROL CENTER':'MAKE IT MOBILE POS'}</span><h1>{label}</h1><p>กำลังเชื่อมต่อข้อมูลร้าน กรุณารอสักครู่</p><div className="loading-dots" aria-hidden="true"><i/><i/><i/></div></section></main>;
}
...
export function FileField({label,onChange,required=false,disabled=false,prominent=false}:{label:string;onChange:(f:File|null)=>void;required?:boolean;disabled?:boolean;prominent?:boolean}){
 ...
 return <div className={`file-field ${prominent?'prominent':''}`}><span className="field-title">{label}{required?' *':''}</span>{prominent&&<p className="upload-lead">แนบภาพให้ชัดและเห็นวันเวลาในสลิปครบถ้วน</p>}<div className="upload-actions"><label className="btn soft"><Camera size={prominent?25:18}/>{prominent?<span><strong>ถ่ายรูปสลิป</strong><small>เปิดกล้องหลัง</small></span>:'ถ่ายรูป'}<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={change} disabled={disabled}/></label><label className="btn secondary"><Upload size={prominent?25:18}/>{prominent?<span><strong>เลือกภาพสลิป</strong><small>จากคลังภาพ</small></span>:'เลือกภาพ'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={change} disabled={disabled}/></label></div>{url&&<img className="upload-preview" src={url} alt="ภาพที่เลือก"/>}<small className="upload-file-name">{file?.name??'รองรับ JPG, PNG หรือ WebP ขนาดไม่เกิน 10 MB'}</small></div>
}
