...
import {report,summaryRows,thaiDate,thaiDay} from './reports';
const saleTime=(value:string)=>new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Bangkok',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(value));
function tables(data:Snapshot,from:string,to:string){
 ...
 {name:'รายการขาย',headers:['เลขบิล','วันที่เวลา','ประเภทลูกค้า','แหล่งเวลา','ชำระโดย','ยอดขาย','ต้นทุน','สถานะ','เหตุผลยกเลิก'],rows:r.sales.map(s=>[s.bill_no,thaiDate(s.created_at),s.customer_type,s.time_source==='slip_ocr'?'OCR จากสลิป':s.time_source==='manual'?'ระบุเอง':'เวลาเดิม',s.payment==='cash'?'เงินสด':'เงินโอน',Number(s.total),Number(s.cost),s.voided_at?'ยกเลิก':'สำเร็จ',s.void_reason??''])},
...
export function salesTemplateRows(data:Snapshot,from:string,to:string){
 const r=report(data,from,to),valid=new Set(r.sales.filter(s=>!s.voided_at).map(s=>s.id));
 return data.items.filter(i=>valid.has(i.sale_id)).map(i=>{const sale=data.sales.find(s=>s.id===i.sale_id)!;return [thaiDay(sale.created_at),saleTime(sale.created_at),i.name,i.quantity,Number(i.price),new Decimal(i.price).times(i.quantity).toNumber(),sale.customer_type==='Unspecified'?'':sale.customer_type,'']});
}
...
export async function exportSalesTemplate(data:Snapshot,from:string,to:string){
 const ExcelJS=await import('exceljs'),workbook=new ExcelJS.Workbook(),sheet=workbook.addWorksheet('Sales Data');
 sheet.addRow(['Date','Time','Product','Quantity','Price','Total','Customer Type','Gender']);
 salesTemplateRows(data,from,to).forEach(row=>sheet.addRow(row));
 const header=sheet.getRow(1);header.font={name:'Prompt',bold:true,color:{argb:'FFFFFFFF'}};header.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF165DFF'}};
 sheet.columns=[{width:14},{width:10},{width:28},{width:12},{width:14},{width:14},{width:18},{width:14}];
 sheet.views=[{state:'frozen',ySplit:1}];sheet.autoFilter={from:{row:1,column:1},to:{row:1,column:8}};
 for(let row=2;row<=sheet.rowCount;row++){sheet.getRow(row).font={name:'Prompt',size:11};for(const col of [4,5,6])sheet.getCell(row,col).numFmt=col===4?'0':'#,##0.00'}
 const buffer=await workbook.xlsx.writeBuffer();download(new Blob([buffer as ArrayBuffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),`Week3_GroupA_Sales_${from}_${to}.xlsx`);
}
...
export async function exportPDF(data:Snapshot,from:string,to:string){
 ...
 tables(data,from,to).forEach((t,index)=>{
  ...
  const widths:(number|'*'|'auto')[]=t.headers.map((_,column)=>column===1?'*':'auto');
  content.push({table:{headerRows:1,widths,body:[t.headers.map(text=>({text,bold:true,color:'#ffffff',fillColor:'#165dff'})),...t.rows.map(row=>row.map(v=>({text:typeof v==='number'?new Intl.NumberFormat('th-TH',{maximumFractionDigits:2}).format(v):String(v)})))]},layout:'lightHorizontalLines'});
 });
